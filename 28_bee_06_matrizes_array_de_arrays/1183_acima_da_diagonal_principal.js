...
function main() {
    ...
    operacao === 'S' ? 
    somar_elementos_acima_da_diagonal_principal(matriz) :
    extrair_media_elementos_acima_da_diagonal_principal(matriz)

    console.log(resultado_operacao.toFixed(1))
}

function somar_elementos_acima_da_diagonal_principal(matriz) {
    let soma = 0

    for(let i = 0; i < matriz.length; i++) {
        for(let j = 0; j < matriz[i].length; j++) {

            if(esta_acima_da_diagonal_principal(i, j)) {
                soma+= matriz[i][j]
            }
        }
    }
...
function extrair_media_elementos_acima_da_diagonal_principal(matriz) {
    const soma = somar_elementos_acima_da_diagonal_principal(matriz)
